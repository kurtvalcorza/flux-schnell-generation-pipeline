...
Rejected requests never import model libraries; the two snapshots are verified and the dataset is validated
before torch, diffusers, transformers, peft or bitsandbytes are imported.
"""
...
from flux_schnell_generation_pipeline import validate_dataset, validate_prompts

PACKAGE = Path(__file__).resolve().parents[1] / "src" / "flux_schnell_generation_pipeline"


def test_package_import_does_not_import_model_libraries(forbid_model_imports):
    import importlib

    import flux_schnell_generation_pipeline

    importlib.reload(flux_schnell_generation_pipeline)
